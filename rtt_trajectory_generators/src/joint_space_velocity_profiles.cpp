...
{
   rtt_tools::useJointVelLims(use_vel_limits, this_taskcontext);
   rtt_tools::useJointAccLims(use_acc_limits, this_taskcontext);
   
   if(use_vel_limits) rtt_tools::getJointVelLims(kdl_chain, max_vel, this_taskcontext);
   if(use_acc_limits) rtt_tools::getJointAccLims(kdl_chain, max_acc, this_taskcontext);
...
void cubicPolynomial::getDesiredJointPos(KDL::JntArray &desired_joint_pos, double &current_trajec_time)
{
...
void cubicPolynomial::getDesiredJointVel(KDL::JntArray &desired_joint_vel, double &current_trajec_time)
{
...
void cubicPolynomial::getDesiredJointAcc(KDL::JntArray &desired_joint_acc, double &current_trajec_time) 
{
...
//Current desired joint positions.
void quinticPolynomial::getDesiredJointPos(KDL::JntArray &desired_joint_pos, double &current_trajec_time)
{
   for(int i=0; i<desired_joint_pos.rows(); ++i)
...
//Current desired joint velocities.
void quinticPolynomial::getDesiredJointVel(KDL::JntArray &desired_joint_vel, double &current_trajec_time)
{
   for(int i=0; i<desired_joint_vel.rows(); ++i)
...
void quinticPolynomial::getDesiredJointAcc(KDL::JntArray &desired_joint_acc, double &current_trajec_time) 
{
   for(int i=0; i<desired_joint_acc.rows(); ++i)
...
//Current desired joint positions.
void bangBang::getDesiredJointPos(KDL::JntArray &desired_joint_pos, double &current_trajec_time)
{
   for(int i=0; i<desired_joint_pos.rows(); ++i)
   {
...
//Current desired joint velocities.
void bangBang::getDesiredJointVel(KDL::JntArray &desired_joint_vel, double &current_trajec_time)
{
   for(int i=0; i<desired_joint_vel.rows(); ++i)
   {
      if(current_trajec_time < t_half[i])
...
//Current desired joint accelerations.
void bangBang::getDesiredJointAcc(KDL::JntArray &desired_joint_acc, double &current_trajec_time)
{
   for(int i=0; i<desired_joint_acc.rows(); ++i)
   {
      if(current_trajec_time < t_half[i])
...
{
   rtt_tools::getJointVelLims(kdl_chain, max_vel, this_taskcontext);
   rtt_tools::getJointAccLims(kdl_chain, max_acc, this_taskcontext);
...
//Current desired joint positions.
void trapezoidal::getDesiredJointPos(KDL::JntArray &desired_joint_pos, double &current_trajec_time) 
{
...
//Current desired joint velocities.
void trapezoidal::getDesiredJointVel(KDL::JntArray &desired_joint_vel, double &current_trajec_time)
{
   for(int i=0; i < switch_time.size(); ++i)
   {
      if(current_trajec_time < switch_time[i])
...
//Current desired joint accelerations.
void trapezoidal::getDesiredJointAcc(KDL::JntArray &desired_joint_acc, double &current_trajec_time) 
{
   for(int i=0; i < switch_time.size(); ++i)
   {
      if(current_trajec_time < switch_time[i])
...
bool getJointSpaceVelProfile(const std::string vel_prof_name, const KDL::Chain &kdl_chain, std::unique_ptr<joint_space_vel_prof::velocityProfile> &vel_prof, RTT::TaskContext *this_taskcontext)
{
