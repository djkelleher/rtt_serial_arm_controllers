#include "rtt_trajectory_generators/random_waypoint_generator.h"

// Create a new instance of RandomWaypointGenerator. The PreOperational argument forces you to make a call to configureHook().
RandomWaypointGenerator::RandomWaypointGenerator(const std::string &name) : RTT::TaskContext(name, PreOperational)
{
  // Add RTT port.
  addPort("trajectory_goal_pose", outPort_TrajectoryGoalPose);
  ...
  addProperty("frame_id", frame_id).doc("reference frame for coordinate system");
}
...
bool RandomWaypointGenerator::configureHook()
{
    ...
    // Parse parameters.
    rtt_tools::getJointPosLims(joint_lower_lims, joint_upper_lims, kdl_chain, this);
    rtt_tools::getFrameID(frame_id, this);
